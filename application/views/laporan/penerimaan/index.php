...
<div style='float:right;'>
	<b>Tgl Dari : </b><?php echo form_input(array('name'=>'tgl_dari','id'=>'tgl_dari','value'=>$tgl_dari,'class'=>'input-small datepicker')) ?><b>&nbsp;Tgl Sampai : </b><?php echo form_input(array('name'=>'tgl_sampai','value'=>$tgl_sampai,'id'=>'tgl_sampai','class'=>'input-small datepicker')) ?>&nbsp;<b> Kantor Cabang : </b> <?php echo form_dropdown('cabang',$dd_cabang,$cabang,'id="id_cabang" class="input-medium"') ?>
	  <button class='btn btn-danger btn-small' onclick='sort_penerimaan()' style='margin-bottom:5px;'><i class='icon-search icon-white'></i></button>&nbsp;
	  <button class='btn btn-inverse btn-small' onclick='cetak_penerimaan()' style='margin-bottom:5px;'><i class='icon-print icon-white'></i></button>	
</div>
...
<script type="text/javascript">
	function sort_penerimaan()
	{
		ajax_sort('<?php echo base_url()."laporan/penerimaan/index?cabang=" ?>'+$('#id_cabang').val()+'&tgl_dari='+$('#tgl_dari').val()+'&tgl_sampai='+$('#tgl_sampai').val());
	}
	function detail(no_resi)
	{
		ajax_modal('<?php echo base_url()."laporan/penerimaan/detail_penerimaan/" ?>'+no_resi,'width:1000px; margin-left:-450px;');
	}
	function cetak_penerimaan()
	{
		window.open('<?php echo base_url()."laporan/penerimaan/index?print=ok&cabang=" ?>'+$('#id_cabang').val()+'&tgl_dari='+$('#tgl_dari').val()+'&tgl_sampai='+$('#tgl_sampai').val());
	}
</script>